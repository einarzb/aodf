...
const padHourInt = (original) => {
 ...
 return  (parseInt(original) > 9) ? `${original}` : `0${original}`;
}
...
export default class SettingsDateRow extends React.Component{
    ...
    constructor(){
        super()
        this.state = {isShowingDate:true, editedTOD:"00:00:00", editedDate:new Date(Date.now())};
    }
    ...
    componentDidMount(){
      ...
      this.setState({
        editedDate,
        editedTOD:`${editedDate.getUTCHours()}:${editedDate.getUTCMinutes()}:${editedDate.getUTCSeconds()}`
      });
    }
    ...
    onDateChange = () => {
        ...
        let dateToSend = `${padHourInt(editedDate.getUTCMonth())}${padHourInt(editedDate.getUTCDate())}${padHourInt(hoursArr[0])}${padHourInt(hoursArr[1])}${editedDate.getUTCFullYear()}`
