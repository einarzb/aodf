...
export default class PasscodeModal extends React.Component{
    ...
    render(){
        let { unSavedChanges, onPasscodeEntered} = this.props;
        return (<SaveModal>
                ...
                <ChangeList>
                  {
                    unSavedChanges.map(change=>{
                      if (typeof change.value === 'object'){
                        ...
                        <ul>
                          {Object.keys(change.value).map((changeKey,index)=>{return (
                            <li key><span>'{changeKey}' to {change.value[changeKey]}</span></li>
                          )}
